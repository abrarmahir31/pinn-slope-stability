...
REM   scripts\run_phase5.bat smoke      GHB smoke under both yield norms
REM   scripts\run_phase5.bat smoke_mc   MC rock-mass smoke - evidence for D-5.2 option a vs c
REM   scripts\run_phase5.bat probe      calibrated criterion, 0.05 steps to SRF 2.0
REM                                     output runs\probe_calibrated - never collected
...
REM D-5.13 CALIBRATED criterion (supervisor, Day 44): plateau factor 5 and an
REM ABSOLUTE admissible floor of 0.50, which put FOS_GHB at the 0.25-grid point
REM SRF 1.500. These thresholds were chosen to land on 1.5; the FOS is therefore
REM calibrated, not predicted, and every report of it must say so.
set ADM_MODE=floor
set ADM_FLOOR=0.50
...
REM collect_results.py excludes smoke runs.
set SMOKE=--smoke --w-yield 1 --admissible-mode %ADM_MODE% --admissible-floor %ADM_FLOOR%
...
if "%1"=="smoke" (
  ...
  echo and the sec per SRF against the calibrated criterion D-5.13.
  exit /b 0
)
...
if "%PLATEAU_FACTOR%"=="CHANGE_ME" (echo DECISION MISSING: PLATEAU_FACTOR - D-5.13 & exit /b 1)
